...
% Durations ---
params.durations.ITI = [10 30]; % [Mu Max]
...
params.durations.total = 1000; % Duration (is sec) of the session
...
while N <= nTrials && toc(ARDUINO.t0) < durSess && ESC
    ...
    ARDUINO = recMVTV2(ARDUINO,durITI);
    [~,~,keyCode] = KbCheck;
    ESC = keyCode(escapeKey) == 0;
...
if exist('Data\UNConditioning','dir') == 0
    mkdir('Data','UNConditioning');
end

if exist(['Data\UNConditioning\AN' anID],'dir') == 0
    mkdir('Data\UNConditioning',['AN' anID]);
end

% Make sure you do not overwrite previous data by creating a different save
% name (append b,c,d,...)
saveName = sprintf('UNCond_AN%s_%s',anID,date);
alphabets = 'bcdefghijklmnopqrstuvwxyz';
idArd = 1;
while exist(['Data\UNConditioning\AN' anID '\' saveName '.mat'],'file') && idArd <= length(alphabets)
    if idArd == 1
...
% save
save(['Data\UNConditioning\AN' anID '\' saveName],'data');
