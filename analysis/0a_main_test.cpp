//g++ -I/opt/homebrew/opt/libmatio/include/ -L/opt/homebrew/Cellar/libmatio/1.5.24/lib/ -o your_program 0a_main_test.cpp -lmatio
...
int main() {
    ...
    if ((matvar->rank) == 2 && (matvar->dims[1]) == 1 && (matvar->class_type) == MAT_C_DOUBLE) {
        ...
        double previous_lever_value = lever_data[0];
        double lever_value = lever_data[0];
        int previous_switch_i = 0;
        int num_switches = 0;
        for (size_t i = 0; i < lever_data.size(); ++i) {
            lever_value = lever_data[i];

            if (lever_value > 2000 && previous_lever_value < 2000 && (i - previous_switch_i) > 1000) {
                cout << "switched from 500 to 2500: " << i << \
                " previous chunk length: " << i - previous_switch_i << " samples = ~" << (i - previous_switch_i)/10000 << "s" << endl;
            ...
            } else if (lever_value < 2000 && previous_lever_value > 2000 && (i - previous_switch_i) > 1000) {
                // cout << "switched from 2500 to 500: " << i << \
                // " previous chunk length: " << i - previous_switch_i << " samples = ~" << (i - previous_switch_i)/10000 << "s" << endl;
                // num_switches = num_switches + 1;

                // vector<double> lever_data_chunk = slice_double_vec(lever_data, previous_switch_i, i);
                // save_double_vec_to_bin(lever_data_chunk, \
                // "./Data/AnB1/B1_20231030_ITI"+to_string(num_switches)+".bin");
