...
def getTrialFrequencies(num_trials, respMTX, binaries_folder):
    '''
    For each trial, get the MATLAB time duration from `respMTX` (index 0 is the trial start time) and get `lever_data` from the 
    ...
    for i in range(0, num_trials-1):
        ...
        trial_lever_data = np.fromfile(binaries_folder+"trial"+str(i)+".bin", dtype=np.double)
        print("Number of lever_data samples: ", trial_lever_data.shape[0])

        frequency = trial_lever_data.shape[0] / (next_trial_start_time - trial_start_time)
