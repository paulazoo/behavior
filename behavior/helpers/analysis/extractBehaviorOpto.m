function dataOptoBeh = extractBehaviorOpto(ANID)
...
%     rtCalcTone = zeros(2,length(toneOrder));
    ...
    for j = 1:length(idxFile)
        ...
        if length(data.params.laser) > 2
            optoType(i) = data.params.laser(3);
        else
            optoType(i) = 1;
        end
        
        if isfield(data.params,'laserLocation')
            laserLocation(i) = data.params.laserLocation;
        else
            laserLocation(i) = 1;
        end
...
%                 end
    ...
    DATES(i) = selDate(i);
    CTL(i) = any(selDateCtl == selDate(i));
    NTRIALS(i,:) = nTrials;
    NEARLY(i,:) = nE;
    PRESS(i,:,:) = nPressTone;
