function [xFit,yFit,B,ev] = fitGoNoGoResponseV2(toneInt,r,showfig)
% Fit data to probability of response in the go/no-go auditory detection task
% use ln(Pr/(1-Pr)) = b0 + b1*s_nogo^n + b2*s_go^n
%
% Inputs:
...
% the data
% 
% NOTE THIS VERSION (2) ASSUMES n = 1 (exposant factor); easier for
% analysis beta values. Also less outputs
...
% Outputs
resEval = 0.01;
xeval = 0.05:resEval:toneInt(end)+0.1;
