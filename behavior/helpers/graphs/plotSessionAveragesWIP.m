...
load('C:\Users\ToneDiscBehavior2\Dropbox (MIT)\VBP\HeadFixedBehavior\Data\ToneDiscrimination\ANGRIN03\ToneDisc_ANGRIN03_03-Feb-2020');
...
% Determine response
for i = 1:4
    HR(i) = sum(H & diffLevel == i)/sum(go & diffLevel == i);
    FAR(i) = sum(FA & diffLevel == i)/sum(noGo & diffLevel == i);
    PERF(i) = (sum(H & diffLevel == i) + sum(CR & diffLevel == i))/(sum(go & diffLevel == i) + sum(noGo & diffLevel == i));
    mRT(i,1) = nanmean(RT(H & diffLevel == i));
    mRT(i,2) = nanmean(RT(FA & diffLevel == i));
end
