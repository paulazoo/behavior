...
%setup nidaq for the laser
%Digital trigger for fast shutters
card.dio_2P3 = daq.createSession('ni');
addDigitalChannel(card.dio_2P3,'Dev1','Port0/Line2','OutputOnly'); % PMT shutters

%Analog signal for laser
card.optosession = daq('ni'); %using newer daq session format as it is marginally faster
% addAnalogOutputChannel(card.optosession, 'Dev1', 1, 'Voltage');
addoutput(card.optosession, 'Dev1', 1, 'Voltage');
card.optosession.Rate = 2000; %changes sampling rate of daq from 1000 (default) to 2000.

%Laser settings
options.laser.mode = 'ramp'; % 
options.laser.laserAmp = 5; % amplitude from 0 to 5.
options.laser.duration = 2.5; % duration of stimulation. Can define more than one.
options.laser.extra_post = 0; % extratime added at end of stimulus
options.laser.extra_pre = 0; %add 10 ms before and after pulse to allow PMT shutters to close/open
[outputdata,~] = initiateLaserDataRH(options);
shutter_flag = 1;
...
%     end
    ...
    if laserIO > 0 && laserMode == 1
        ...
        fprintf('LASER!')
        outputSingleScan(card.dio_2P3,1); %Close shutters
        shutter_flag = 0;
        preload(card.optosession, outputdata{1});
%         queueOutputData(card.optosession, outputdata{1});
        start(card.optosession, "continuous");
%         card.opto.startBackground();
    end
    
    % FOREPERIOD ==================================
    if shutter_flag == 0
        outputSingleScan(card.dio_2P3,0) %Open shutters
        shutter_flag = 1;
    end
...
%                 queueOutputData(card.optosession, outputdata{1});
                start(card.optosession, "continuous");
%                 card.optosession.startBackground();
                MTXTrialType(N,5) = 1;
%                 fprintf(ardOut,'A'); % Send a pulse to digidata
