...
MVTBL = referenceMVT(ARDUINO,100);
pause(2);
...
while N <= nTrials && ESC
    ...
    while isMVT && ESC
        ...
        [ARDUINO,isMVT] = detectMVT(ARDUINO,paramsDetectMVT,escapeKey);
        if isMVT
            if FIRSTFLAG
    ...
    [ARDUINO,isMVT,ESC] = detectMVT(ARDUINO,paramsDetectMVT,escapeKey);
    if isMVT
...
%                 durPost = dur_consumption;
        ...
        ARDUINO = recMVT(ARDUINO,dur_consumption);
        fprintf(ardOut,'I'); % LED ON
...
%         toc
    ...
    % Stop if nM > maxMiss 
%     if nM > maxMiss
    if GetSecs - ARDUINO.t0 > tNextMessage
        str = strPerfo(respMTX,MTXTrialType,N);
        subjectStr = sprintf('%s: elapsed time %3.1f min',anID,(GetSecs - ARDUINO.t0)/60);
